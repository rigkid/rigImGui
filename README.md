...
Named dock layouts, Photoshop-style: snapshot the current layout **and window visibility** under a name, switch between snapshots from **View → Workspace**. Each workspace is a `<name>.ini` under `AppPaths::getWorkspacesDir()` (`data/user/workspaces/`) — ImGui dock data plus a `[RigVisibility][Windows]` section. The live session keeps autosaving to `imgui.ini` there.
...
Menu: **View → Workspace** lists saved workspaces (checkmark on the active one), **Save Workspace As...** prompts for a name, **Delete Workspace** confirms before removing (Standard is omitted). Loading a workspace that includes `[RigVisibility]` hides every registered window, then shows only those marked visible — panels not in the snapshot stay hidden.
